...
uint32 search(tst_db *db, uint32 node, const char* s, int d,int len_of_s){
	...
	//printf("###%c,%c\n",c,data[node].c);
	//printf("~~~%d,%d\n",d,strlen(s));
	if(c < data[node].c)   return search(db,data[node].left,s,d,len_of_s);
	else if(c > data[node].c)  return search(db,data[node].right,s,d,len_of_s);
	else if(d < len_of_s-1) return search(db,data[node].mid,s,d+1,len_of_s);
	else return node; 	
}
...
tst_db* tst_open(const char* full_file_name){
	...
	while(1){
		...
		//printf("pos:%lu\n",tmp_cur);
		ct_read = fread(&tmp_len_of_key,sizeof(int),1,db->read_file_ptr);
		if(ct_read<=0){
			break;
		}
