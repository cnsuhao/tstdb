...
void dfs(tst_db *db, uint32 node, char* result, int* result_size,
         char key_buf[],int d,int limit,enum SortingOrder sorting_order)
{
		uint32 first,second;
		//printf("--dfs--\n");
        if(node==0)
...
static
void dfs_less(tst_db *db, uint32 node, const char *key, char* result, int * result_size, 
...
static
void dfs_greater(tst_db *db, uint32 node, const char *key, char* result, int * result_size, 
               char key_buf[],int d,  int limit)
{
        char c ;
        if(node == 0)
...
void tst_prefix(tst_db *db, const char* prefix,char* result,
               int* result_size,int limit, enum SortingOrder sorting_order)
{
