...
static uint32
insert(tst_db * db, uint32 node, const char *s, uint64 value, int d, int len_of_s, uint32 parent)
{
	char c = s[d];
	uint32 x = node, no;
	if (x == 0) {
...
void
tst_put(tst_db * db, const char *key, uint64 value)
{
	int len_of_key = strlen(key);
	if (len_of_key <= 0)
		return;
...
static
    void
append_result(const char *key, char *result, int *result_size)
{
	//printf("append: %s\n",key);
	strcat(result, key);
	strcat(result, "\r\n");
...
static
    void
dfs(tst_db * db, uint32 node, char *result, int *result_size, char key_buf[], int d, int limit, enum SortingOrder sorting_order)
{
	uint32 first, second;
	//printf("--dfs--\n");
	if (node == 0)
...
static
    void
dfs_less(tst_db * db, uint32 node, const char *key, char *result, int *result_size, char key_buf[], int d, int limit)
{
	char c;
	if (node == 0)
		return;
...
static
    void
dfs_greater(tst_db * db, uint32 node, const char *key, char *result, int *result_size, char key_buf[], int d, int limit)
{
	char c;
	if (node == 0)
		return;
...
void
tst_less(tst_db * db, const char *key, char *result, int *result_size, int limit)
{
	int len_of_key = strlen(key);
...
void
tst_greater(tst_db * db, const char *key, char *result, int *result_size, int limit)
{
	int len_of_key = strlen(key);
	char base_key[MAX_KEY_SIZE] = { 0 };
	int root;
...
void
tst_prefix(tst_db * db, const char *prefix, char *result, int *result_size, int limit, enum SortingOrder sorting_order)
{
