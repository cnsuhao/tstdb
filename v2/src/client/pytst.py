...
"""
client module for tstdb (key-value store)

Overview
========

See U{the tstdb homepage<http://code.google.com/p/tstdb>} for more about tstdb.
...
__author__    = "Sean Reifschneider <jafo-memcached@tummy.com>; Sun, Junyi<ccnusjy@gmail.com>"
__version__ = "1.48"
...
def TSTClient(host='localhost',port=8402):
    return Client([str(host)+':'+str(port)])
...
class Client(local):
    ...
    def _expectkeys(self, server, line=None):
        if not line:
            line = server.readline()

        if line and line[:4] == 'KEYS':
            resp,r_size,bytes_len = line.split()
            r_size = int(r_size)
            bytes_len = int(bytes_len)
            return (r_size, bytes_len)
        else:
            return (None, None)
...
class _Host(object):
    ...
    def __init__(self, host, debug=0, dead_retry=_DEAD_RETRY,
        ...
        if hostData.get('proto') == 'unix':
            ...
            self.port = int(hostData.get('port', 8402))
